...
public class PiscinaConfiguration : IEntityTypeConfiguration<Piscina>
{
    public void Configure(EntityTypeBuilder<Piscina> builder)
    {
        ...
        // Sentinel explícito: los valores reales del enum empiezan en 1 (Piscina), así que 0 nunca
        // es un valor válido — sirve como "no asignado" sin ambigüedad con HasDefaultValue.
        builder.Property(p => p.TipoBanio).HasColumnName("tipo_banio").HasConversion<int>()
            .HasDefaultValue(Termales.Entities.Enums.TipoBanio.BanoPrivado).HasSentinel(default(Termales.Entities.Enums.TipoBanio));
    }
}
