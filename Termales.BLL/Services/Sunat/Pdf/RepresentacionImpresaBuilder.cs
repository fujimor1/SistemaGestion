        ...
        var documento = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(col =>
                {
                    ...
                    col.Item().PaddingTop(2).Text($"SON: {TicketFormato.MontoEnLetras(comprobante.Total)}").FontSize(8).Italic();
                    col.Item().Text(TicketFormato.FormaDePagoTexto(comprobante.MetodoPago, comprobante.Total, comprobante.MontoEfectivoMixto)).FontSize(8);
